...
# Get the differential motifs per cell type
markersAM <- getMarkerFeatures(
  ArchRProj = project,
...
markerList <- getMarkers(markersAM, cutOff = "FDR <= 0.01")

# Combine all data frames from markerList into one with an additional CellType column
merged_df <- do.call(rbind, lapply(names(markerList), function(ct) cbind(as.data.frame(markerList[[ct]]), CellType = ct)))

# Select the columns
merged_df <- merged_df[, c("name", "FDR", "MeanDiff", "CellType")]

# Write the merged data frame to a TSV file
write.table(merged_df, paste0(sannot_dir, "merged_markerList.tsv"), sep = "\t", row.names = FALSE, quote = FALSE)
