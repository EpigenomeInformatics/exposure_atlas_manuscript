...
genes <- c(
  "HLA-E","HLA-DRA", "HLA-DPA1", "HLA-DRB1","HLA-DRB5", "HLA-DQB1",
   "CCL2", "CD4", "TNF", "IL6", "IL1B", "TXNIP", "CD74", "EIF1", "ZFP36", "S100A8", "S100A9", "XAF1", "IFI6",
  "OAS1", "OAS2", "IFITM3", "LGALS1", "PLBD1", "S100A12", "FCGR3A", "CCL3",
  "CCL4", "HLA-DPB1", "HLA-DMA", "S100A9", "IRF7", "CXCL2", "ACTB", "CLU",
  "PTMA", "ITM3", "DYF", "MX1", "RNY1", "LGALS1", "PPIA"
)[1:6]
...
motif_name <- c("CEBPD","JUN", "FOS", "FOSL2", "NFKB1", "NFKB2", "RELA", "RELB","SPIB","FOSL2::JUN"
,"CREB1","BATF","BATF::JUN","SPIC")[1]
...
interesting_motifs <- names(motifs)[grepl("JUN|RELA", names(motifs), ignore.case = TRUE)]
motif_positions <- motifmatchr::matchMotifs(
...
peaks_matched <- list(
  Peaks = peaks,
  cCREs = allDiffPeaks,
  JUN_Motif = getPeakSet(project)[assay(motif_positions)[, grep("JUN", colnames(motif_positions), ignore.case = TRUE)[1]]],
  RELA_Motif = getPeakSet(project)[assay(motif_positions)[, grep("RELA", colnames(motif_positions), ignore.case = TRUE)[1]]]
)
plot <- plotBrowserTrack(
  ArchRProj = project,
  features = peaks_matched,
...
plotPDF(
  plotList = plot,
  name = "Plot-Motif-JUN-RELA-Track-plots-Mono-CD4.pdf",
  ArchRProj = project,
  addDOC = FALSE, width = 5, height = 5
)
