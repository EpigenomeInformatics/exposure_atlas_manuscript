...
#####################################################################
# 07_1_run_ChrAccR_flu.R
# created on 2025-08-04 by Irem Gunduz
...
sampleannot <- read.delim("/icbb/projects/igunduz/sampleannot.tsv") %>%
  dplyr::filter(sample_exposure_group %in% c("Influenza_ctrl", "Influenza_d3", "Influenza_d6", "Influenza_d30"))
sampleannot$fragmentFiles <- gsub(x = sampleannot$fragmentFiles, pattern = ".bed", replacement = ".tsv.gz")
sampleannot$donorId <- sub(".*_(flu[0-9]+)_.*", "\\1", sampleannot$sampleIdCol)
...
lapply(cells, function(cell) {
  ...
  setConfigElement("differentialAdjColumns", "donorId")
  ...
  setConfigElement("normalizationMethod", "percentile")
