...
n <- 1
source("/icbb/projects/igunduz/itLSI.r")
tcell_se <- readRDS("/icbb/projects/igunduz/DARPA/scATAC_TME_TCells_SummarizedExperiment.final.rds")
seATAC <- seATAC2 <- tcell_se
seATAC <- itLsi(seATAC)
saveRDS(seATAC, file = "/icbb/projects/igunduz/DARPA/scATAC_TME_TCells_itlsi_result.rds")

projectBulkATAC_se <- function(
    seATAC = NULL,
    seATAC2 = NULL,
  ...
  ArchR:::.validInput(input = seATAC2, name = "seATAC2", valid = c("SummarizedExperiment"))
