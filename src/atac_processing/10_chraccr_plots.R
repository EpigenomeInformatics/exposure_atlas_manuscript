...
set.seed(12)
source("/icbb/projects/igunduz/sc_epigenome_exp/utils/lola.R")
...
outputDir <- "/icbb/projects/igunduz/DARPA_analysis/chracchr_run_011023/ChrAccRuns_covid_2023-10-02/"
cell <- "Mono_CD14"
plot_dir <- "/icbb/projects/igunduz/DARPA_analysis/chracchr_run_011023/plots/"
cor_list <- list()
...
lolaDb <- "/icbb/projects/igunduz/annotation/lolaDB/hg38/"
lolaDb <- RnBeads::loadLolaDbs(lolaDb)
lola_path <- "/icbb/projects/igunduz/DARPA_analysis/LOLA/"
if(!dir.exists(lola_path)){dir.create(lola_path)}
...
diff  <- prepareDARforPlot(cell,outputDir,3,"archrPeaks",padj=0.01) #get the C19_severe
...
#fragmentGR <- getFragmentGrl(ds, getSamples(ds)) 
#counts <- getFragmentsPerPeak(ds, "archr_peaks")
#counts <- regionSetCounts(ds,fragmentGR,bySample=FALSE)
...
coords <- data.table::as.data.table(getCoord(ds, "archr_peaks")) %>%
 ...
 # Apply k-means clustering
  ...
  col_ha <- HeatmapAnnotation(Exposure = col_ann$Exposure,
                              col = list(Exposure = c("C19_ctrl" = "#4F609C", 
                                                     "C19_mild" = "#C43E96", 
                                                     "C19_mod" = "#06948E", 
                                                     "C19_sev" = "#C03830")))
...
dend_cols <- cluster_within_group(counts_ordered, group_order)

dend_cols <- cluster_within_group(counts_ordered, col_ann$Exposure)#(gsub("^([^_]*_[^_]*).*", "\\1", colnames(counts_ordered))))
#dend_cols <- cluster_between_groups(rev(counts_ordered), rev(col_ann$Exposure))#(gsub("^([^_]*_[^_]*).*", "\\1", colnames(counts_ordered))))
  ...
  ht <- Heatmap(
    ...
    #row_split = row_ann_ordered,
    column_title_side = "top",
    top_annotation = col_ha#,
    #right_annotation = row_ha,
    #width = unit(nrow(counts_ordered) / 2, "cm")
  )

pdf(paste0("/icbb/projects/igunduz/pheatmap_",cell,"_samplewise_C19_diff.pdf"), width = 20, height = 20)
draw(ht)
dev.off()
